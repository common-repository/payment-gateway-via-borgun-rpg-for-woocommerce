...
define( 'BORGUN_RPG_VERSION', '1.0.27' );
    ...
    if( !empty($_POST['data'])) {
      ...
      if($order_id){
        ...
        if(!empty($borgun_tds_method) && !empty($order)){
          delete_post_meta($order_id, 'borgun_tds_enrollment');
          ...
          if( !empty($api_response) && isset( $api_response->RedirectToACSForm ) && !empty($api_response->RedirectToACSForm) ){
            ...
            update_post_meta( $order_id, 'borgun_secure_form', $html);
...
function borgun_rpg_admin_assets() {
  wp_enqueue_script( 'borgun-rpg-notice-update', BORGUN_RPG_URL .'assets/admin/js/notice_update.js', array( 'jquery' ), '1.0', true  );
}
...
function borgun_rpg_assets() {
  if (function_exists('is_woocommerce')){
    if( is_checkout() ){
      wp_register_style( 'borgun-rpg-styles', BORGUN_RPG_URL . 'assets/css/styles.css' );
